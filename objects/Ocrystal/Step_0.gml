    ...
    if (instance_exists(Ocherry)) {
        ...
        if (_distance <= attack_range) {
            ...
            if (_use_special) {
                ...
                switch(_max_phases) {
                    case 1:
                        sprite_index = ScrystalA;  // Level 1: Basic attack
                        break;
                    case 2:
                        sprite_index = ScrystalA2;  // Level 2: second combo 
                        break;
                    case 3:
                        sprite_index = ScrystalA3; // Level 3+: Full combo
                        break;
                }
...
if (!place_meeting(x, y + 1, wall)) {
    ...
    if (!attacking && !invincible && !jump_attacking && !using_special) {
        if (vsp < 0) {
            ...
            sprite_index = ScrystalJ;
        } else {
            // Going DOWN - use fall sprite  
            sprite_index = ScrystalF;
        }
    }
} else {
    grounded = true;
