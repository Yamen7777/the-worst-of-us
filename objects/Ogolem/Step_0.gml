...
// Hover: snap to hover_height above ground if within range
if (variable_instance_exists(id, "hover_height")) {
    var _check_y = y;
    var _ground_found = false;
    while (_check_y < y + hover_height + 10 && !_ground_found) {
        var _on_any_wall = false;
        for (var i = 0; i < array_length(wall); i++) {
            if (place_meeting(x, _check_y, wall[i])) {
                _on_any_wall = true;
                break;
            }
        }
...
//animation
var _grounded_via_hover = false;
if (variable_instance_exists(id, "hover_height")) {
    // Check if there's ground within hover_height below
    var _check_y = y + 1;
    var _found_ground = false;
    while (_check_y < y + hover_height && !_found_ground) {
        var _on_any_wall = false;
        for (var i = 0; i < array_length(wall); i++) {
            if (place_meeting(x, _check_y, wall[i])) {
                _on_any_wall = true;
