...
if (!attacking && attack_cooldown == 0) {
    ...
    if (instance_exists(Ocherry)) {
        ...
        if (_distance <= attack_range) {
            // Start attack
            attacking = true;
            if(shuild) sprite_index = Sgolem_OcrystalA;
			else sprite_index = Sgolem_BcrystalA;
            image_index = 0;
...
if (attacking) {
    hsp = 0;
    if (image_index >= 6 && !attack_created) {
        attack_created = true;
        var _attackX = x + (face * 165);
        with (instance_create_layer(x, y, "bullets", Ogolem_crystalAttack)) {
            if(other.shuild) sprite_index = Sgolem_OcrystalAttack;
			else sprite_index = Sgolem_BcrystalAttack;
...
if (!place_meeting(x, y + 1, wall)) {
    ...
    if (!attacking && !invincible) {
        if(shuild) sprite_index = Sgolem_Ocrystal;
		else sprite_index = Sgolem_Bcrystal;
        image_speed = 0;
    ...
    if (!attacking && !invincible) {
        ...
        if (hsp == 0) {
            ...
            if(shuild) sprite_index = Sgolem_OcrystalW;
			else sprite_index = Sgolem_BcrystalW;
        }
    }
}
...
if (invincible) and (!attacking) {
    invincible_timer--;
    image_alpha = 0.9;
    if(shuild) sprite_index = Sgolem_OcrystalH;
	else sprite_index = Sgolem_BcrystalH;
    image_speed = 1;
	image_blend = c_red;
    ...
    if (instance_exists(obj_type)) {
        ...
        if (damager != noone) {
            ...
            // NORMAL CASE: All other damage sources
                ...
                if (ds_list_find_index(damaged_by_list, damager) == -1) {
                    var _is_heavy = variable_instance_exists(damager, "heavy") && damager.heavy == true;
                    
                    // Shield check - block damage if shuild is active
                    if (shuild && !_is_heavy) {
                        show_damage_number(x, y, 0, -370, "block");
                        ds_list_add(damaged_by_list, damager);
                        invincible = true;
                        invincible_clear_timer = invincible_clear_time;
                    } else {
                        ...
                        // break shuild if hit with heavy attack
                        if (_is_heavy && shuild) shuild = false;
...
if(!shuild)
{
	...
	if(shuild_timer <= 0) 
	{
		shuild_timer = shuild_time;
		shuild = true;
	}
}

//TEST shuild off 
if(keyboard_check_pressed(ord("B"))) shuild = !shuild;