...
if (jump_attacking) {
    ...
    if (image_index >= 5 && !attack_created[0]) {
        ...
        with (instance_create_layer(x, y, "bullets", Omage_attacks)) {
            sprite_index = Smage_attack2; // Temporary - until SfireJA sprite has its own hitbox
    ...
    if (instance_exists(Ocherry)) {
        ...
        if (_distance <= attack_range) {
            ...
            if (_use_special) {
                ...
                switch(_max_phases) {
                    case 1:
                        sprite_index = SmageA;  // Level 1: Basic attack
                        break;
                    case 2:
                        sprite_index = SmageA2;  // Level 2: second combo 
                        break;
                }
...
if (using_special) {
    ...
    if (image_index >= 6 && !attack_created[0]) {
        ...
        with (instance_create_layer(x, y, "bullets", Omage_attacks)) {
            sprite_index = Smage_attack3; // Temporary until you make the special sprite
...
if (attacking) {
    ...
    var _in_lunge_phase = (_max_phases >= 2 && image_index >= 10 && image_index < 13);
    if (!_in_lunge_phase) {
        hsp = 0;
    }
    
    // ATTACK 1: Always available (Level 1+)
    if (_max_phases >= 1 && image_index >= 8 && !attack_created[0]) {
        attack_created[0] = true;
        
        with (instance_create_layer(x, y, "bullets", Omage_attacks)) {
            sprite_index = Smage_attack1;
            damage = other.current_damage;
            image_xscale = other.face;
        }
    }
