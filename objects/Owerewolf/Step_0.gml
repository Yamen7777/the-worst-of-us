/// @description player movment

//custom functions
function onground(_val = true)
{
	if _val == true
	{
		ground = true;
		cayoteHT = cayoteHF;
	}
	else
	{
		ground = false;
		MyFloorPlat = noone;
		cayoteHT = 0;
	}
}
function ssplat_check(_x,_y)
{
	//create a return veriable
	var _rtrn = noone;
	
	//check for normal collision if moving downward
	if (vsp >= 0) and (place_meeting(_x,_y,Ossplat))
	{
		//create a ds list for simesolid plarform colliding instances
		var _list = ds_list_create();
		var _listSize = instance_place_list(_x,_y,Ossplat,_list,false);
		
		//loop throurh the colliding instances to find the top platform bellow the player
		for(var i = 0; i < _listSize; i++)
		{
			var _listIsnt = _list[| i];
			if (_listIsnt != forgetssp) and (floor(bbox_bottom) <= ceil(_listIsnt.bbox_top - _listIsnt.vsp))
			{
				//return the id of the semisolid platform
				_rtrn = _listIsnt;
				//exit the loop early
				i = _listSize;
			}
		}
		//destroy the list to free memory
		ds_list_destroy(_list);
	}
	//return our variable
	return _rtrn;
}

//player input
general_input();

//hitstun timer (counts down every step)
if (hitstun_time > 0)
{
	image_alpha = 0.9;
	image_blend = c_red;
	hitstun_time--;
}
else
{
	image_alpha = 1;
	image_blend = c_white;
}



//moving platform collision
#region
var _rightWall = noone;
var _leftWall = noone;
var _bottomWall = noone;
var _topWall = noone;
var _list = ds_list_create();
var _listSize = instance_place_list(x,y,OmovingPlat,_list,false);

//loop through all colliding moving platform
for (var i = 0; i < _listSize; i++)
{
	var _listinst = _list[| i];
	
	//find the closest wall
	//right walls
	if _listinst.bbox_left - _listinst.hsp >= bbox_right-1
	{
		if (!instance_exists(_rightWall)) or (_listinst.bbox_left < _rightWall.bbox_left)
		{
			_rightWall = _listinst; 
		}
	}
	//left walls
	if _listinst.bbox_right - _listinst.hsp <= bbox_left+1
	{
		if (!instance_exists(_leftWall)) or (_listinst.bbox_right > _leftWall.bbox_right)
		{
			_leftWall = _listinst;
		}
	}
	//bottom wall
	if _listinst.bbox_top - _listinst.vsp >= bbox_bottom-1
	{
		if (!instance_exists(_bottomWall)) or (_listinst.bbox_top < _bottomWall.bbox_top)
		{
			_bottomWall = _listinst;
		}
	}
	//top wall
	if _listinst.bbox_bottom - _listinst.vsp <= bbox_top+1
	{
		if (!instance_exists(_topWall)) or _listinst.bbox_bottom > _topWall.bbox_bottom
		{
			_topWall = _listinst;
		}
	}
}
ds_list_destroy(_list);

//get pushed
//right wall
if instance_exists(_rightWall)
{
	var _rightDist = bbox_right - x;
	x = _rightWall.bbox_left - _rightDist;
}
//left wall
if instance_exists(_leftWall)
{
	var _leftDist = x - bbox_left;
	x = _leftWall.bbox_right + _leftDist;
}
//bottom wall
if instance_exists(_bottomWall)
{
	var _bottomDist = bbox_bottom - y;
	y = _bottomWall.bbox_top - _bottomDist;
}
//top wall (includes, collision for polish and crounching features)
if instance_exists(_topWall)
{
	var _topDist = y - bbox_top;
	var _targetY = _topWall.bbox_bottom + _topDist; 
	//check if there isnt a wall in the way
	if !place_meeting(x,_targetY,Owall)
	{
		y = _targetY;
	}
}
#endregion

//state
STATE();

//reset invincible
if (transform && invincible && hitstun_time <= 0) {
    invincible = false; // Reset invincible if stuck
}

if(sprite_index = SwerewolfD) and (!dead)
{
	dead = true;

	audio_play_sound(SNdeath,10,false);
}

//check if we are crushed 
if place_meeting(x,y,Owall)
{
	STATE = STATE_DEAD;
}

// SPRITE CONTROL
if(transform)
{
	//death
	if(STATE = STATE_DEAD)
	{
	    sprite_index = SPwerewolfD;
	}
	//transforming
	else if(transforming)
	{
		//dont move while transforming
		hsp = 0;
		vsp = 0;
		sprite_index = SPwerewolfSP;
		if(image_index == 6)
		{
			transforming = false;
		}
	}
	//hitstun
	else if (hitstun_time > 0)
	{
		sprite_index = SPwerewolfHT;
	}
	//jumping
	else if (!ground)
	{
		if(sliding)
		{
			sprite_index = SPwerewolfWJ;
		}
		else
		{
			if(vsp < 0)
			{
				sprite_index = SPwerewolfJ;
			}
			if(vsp > 0)
			{
				sprite_index = SPwerewolfJ2;
			}
		}
	}
	//attacks
	else if (hold_attack || hold_released) {
	    sprite_index = SPwerewolfAT5;
    
	    if (hold_attack && !hold_released) {
	        // Freeze animation based on charge level
	        var charge_level = floor(hold_time / hold_scratch);
        
	        if (charge_level == 0) {
	            image_index = 0; // Stay at frame 0 (no scratch yet)
	        } else if (charge_level == 1) {
	            image_index = 1; // Stay at frame 1 (1 scratch)
	        } else if (charge_level == 2) {
	            image_index = 2; // Stay at frame 2 (2 scratches)
	        } else if (charge_level >= 3) {
	            image_index = 3; // Frame 3 (full charge - will auto release)
	        }
        
	        image_speed = 0; // Freeze animation while charging
	    } else if (hold_released) {
	        image_speed = 1; // Resume animation after release
	    }
	}
	//run attack
	else if (run_attack)
	{
		sprite_index = SPwerewolfAT4;
	}
	//normal attacks
	else if (attack1) {
	    sprite_index = SPwerewolfAT1;
	    if (attack1_started) {
	        image_index = 0;
	        attack1_started = false;
	    }
	} else if (attack2) {
	    sprite_index = SPwerewolfAT2;
	    if (attack2_started) {
	        image_index = 0;
	        attack2_started = false;
	    }
	} else if (attack3) {
	    sprite_index = SPwerewolfAT3;
	    if (attack3_started) {
	        image_index = 0;
	        attack3_started = false;
	    }
	}
	//crouching 
	else if (crouching) sprite_index = SPwerewolfC;
	//running
	else if (abs(hsp) > 0) and (run)
	{
		sprite_index = SPwerewolfR;
	}
	//walking
	else if (abs(hsp) > 0)
	{
		sprite_index = SPwerewolfW;
	}
	//running
	else if (abs(hsp) >= walkSP[1]) sprite_index = SPwerewolfW; 
	//no moving
	else if (hsp == 0) sprite_index = SPwerewolf;
	//collision mask
	mask_index = SPwerewolf;
	if (crouching) mask_index = SPwerewolfC;
}
else //normal form
{
	//fusing
	if(STATE = STATE_FUSE)
	{
		sprite_index = SwerewolfF;
	}
	//death
	else if(STATE = STATE_DEAD)
	{
	    sprite_index = SwerewolfD;
	}
	//transforming
	else if(transforming)
	{
		//dont move while transforming
		hsp = 0;
		vsp = 0;
		sprite_index = SwerewolfSP;
		if(image_index == 6)
		{
			transform = true;
			image_index = 0;
		}
	}
	//hitstun
	else if (hitstun_time > 0)
	{
		sprite_index = SwerewolfHT;
	}
	//jumping
	else if (!ground)
	{
		if(sliding)
		{
			sprite_index = SwerewolfWJ;
		}
		else
		{
			if(vsp < 0)
			{
				sprite_index = SwerewolfJ;
			}
			if(vsp > 0)
			{
				sprite_index = SwerewolfJ2;
			}
		}
	}
	//attacks
	else if (hold_attack || hold_released) {
	    sprite_index = SwerewolfAT5;
    
	    if (hold_attack && !hold_released) {
	        // Freeze animation based on charge level
	        var charge_level = floor(hold_time / hold_scratch);
        
	        if (charge_level == 0) {
	            image_index = 0; // Stay at frame 0 (no scratch yet)
	        } else if (charge_level == 1) {
	            image_index = 1; // Stay at frame 1 (1 scratch)
	        } else if (charge_level == 2) {
	            image_index = 2; // Stay at frame 2 (2 scratches)
	        } else if (charge_level >= 3) {
	            image_index = 3; // Frame 3 (full charge - will auto release)
	        }
        
	        image_speed = 0; // Freeze animation while charging
	    } else if (hold_released) {
	        image_speed = 1; // Resume animation after release
	    }
	}
	//run attack
	else if (run_attack)
	{
		sprite_index = SwerewolfAT4;
	}
	//normal attacks
	else if (attack1) {
	    sprite_index = SwerewolfAT1;
	    if (attack1_started) {
	        image_index = 0;
	        attack1_started = false;
	    }
	} else if (attack2) {
	    sprite_index = SwerewolfAT2;
	    if (attack2_started) {
	        image_index = 0;
	        attack2_started = false;
	    }
	} else if (attack3) {
	    sprite_index = SwerewolfAT3;
	    if (attack3_started) {
	        image_index = 0;
	        attack3_started = false;
	    }
	}
	//poke
	else if(STATE = STATE_POKE)
	{
	    sprite_index = SwerewolfDS;
	}
	//crouching 
	else if (crouching) sprite_index = SwerewolfC;
	//running
	else if (abs(hsp) > 0) and (run)
	{
		sprite_index = SwerewolfR;
	}
	//walking
	else if (abs(hsp) > 0)
	{
		sprite_index = SwerewolfW;
	}
	//running
	else if (abs(hsp) >= walkSP[1]) sprite_index = SwerewolfW; 
	//no moving
	else if (hsp == 0) sprite_index = Swerewolf;
	//collision mask
	mask_index = Swerewolf;
	if (crouching) mask_index = SwerewolfC;
}
