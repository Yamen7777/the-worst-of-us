...
//health / damage
hp = 100;
hitstun_time = 0; // frames of stun / invincibiliity after taking damage
...
attack1_duration = 35; // 6 frames at 12fps (7 * 60/12 = 35)
attack2_duration = 50; // 10 frames at 12fps (10 * 60/12 = 50)
attack3_duration = 50; // 10 frames at 12fps (10 * 60/12 = 50)
...
hold_attack = false;
hold_time = 0;
hold_blade = 24; // Frames to charge one blade
max_blades = 8; // Maximum blades that can be charged
blade_release = 0;
blade_release_timer = 0;
blade_release_delay = 5; // Frames between each blade release
HA = false; //hold attack check

//super
blue_spirit = 0;
super_attack = false;
super_duration = 35; // 6 frames at 12fps (7 * 60/12 = 35)
...
STATE_FREE = function()
{
	...
	if(sprite_index == SyokaiW) and (!_walking)
	{
	...
	if(sprite_index == SyokaiR) and (!_walking)
	{
	...
	else
	{
		...
		if(run) var maxspd = 28; //max speed when the player is running
		else maxspd = 20; // Max speed when the player is walking
	...
	if (dash and canDash) 
	{
		...
		onground(false);
	    canDash = false;
		audio_sound_pitch(SNdash,random_range(0.9,1));
	...
	if(room != Ressance)
	{
		...
		//bluefire par
		if(!instance_exists(ObluefirePar))
		{
			instance_create_layer(490,300,layer,ObluefirePar);
		}
	}
	//camp the blue spirit
	blue_spirit = clamp(blue_spirit,0,8);
	...
	attack = (attack1 || attack2 || attack3 || attack_timer > 0);
	...
	// IDLE - Can start attack 1
	    ...
	    // Charging up
	    else if (HLMB) and (ground) and (image_index < 50) {
	        hold_time++;
			hsp = 0;
	        // Cap at max blades
	    ...
	    // Released the button - calculate how many blades to release
	        ...
	        if (hold_attack) {
				...
				//create hold fire effect
				if(face ==  1) var _bladeX =  140;
				if(face == -1) var _bladeX = -140;
				with(instance_create_layer(Ocherry.x+_bladeX,Ocherry.y-175,"bullets",Oblue_effect)) 
				{
					sprite_index = Sblue_effect4;
					charge = other.charge;
					image_xscale = other.charge * other.face;
	    ...
	    if (LMB) and (ground) {
	        ...
	        queued_attack2 = false;
	        queued_attack3 = false;
			//create attack 1 fire
			if(face ==  1) var _bladeX =  140;
			if(face == -1) var _bladeX = -140;
			with(instance_create_layer(Ocherry.x+_bladeX,Ocherry.y-175,"bullets",Oblue_effect)) 
				{
					sprite_index = Sblue_effect1;
					image_xscale = other.face;
				}
	        // Play attack 1 sound
			audio_sound_pitch(SNfire,random_range(0.9,1.1));
	        audio_play_sound(SNfire, 4, false);
	    }
	}
	...
	if (attack1 && attack_timer == 0) {
	    ...
	    if (queued_attack2) {
	        ...
	        combo_window_timer = combo_window_start;
	        queued_attack2 = false;
			//create attack 2 fire
			if(face ==  1) var _bladeX =  140;
			if(face == -1) var _bladeX = -140;
			with(instance_create_layer(Ocherry.x+_bladeX,Ocherry.y-175,"bullets",Oblue_effect)) 
				{
					sprite_index = Sblue_effect2;
					image_xscale = other.face;
				}
	        // Play attack 2 sound
	        audio_sound_pitch(SNfire,random_range(0.9,1.1));
	...
	if (attack2 && attack_timer == 0) {
	    ...
	    if (queued_attack3) {
	        ...
	        queued_attack3 = false;
			//create attack 3 fire
			repeat(4)
			{
				if(face ==  1) var _blade_X =  random_range(120,190);
				if(face == -1) var _blade_X = -random_range(120,190);
				var _blade_Y = random_range(50,140); 
				with(instance_create_layer(Ocherry.x+_blade_X,Ocherry.y-_blade_Y,"bullets",Oblue_effect))
					{
						sprite_index = Sblue_effect3;
						image_xscale = Oyokai.face;
					}
			}
	        // Play attack 3 sound
	        audio_sound_pitch(SNfire,random_range(0.9,1.1));
	        audio_play_sound(SNfire, 1, false);
	    } else {
	...
	//use ability to bring back the blades 
	// Check if E is pressed
	if (super) and (ground) and (!super_attack) and (blue_spirit > 0)
	{
	    super_attack = true;
		//create attack 1 fire
			if(face ==  1) var _bladeX =  140;
			...
			with(instance_create_layer(Ocherry.x+_bladeX,Ocherry.y-175,"bullets",Oblue_effect)) 
				{
					blue_spirit = Oyokai.blue_spirit;
					sprite_index = Sblue_effect5;
					image_xscale = other.face;
				}
	        // Play attack 1 sound
			audio_sound_pitch(SNfire,random_range(0.7,0.8));
...
STATE_DEAD = function() {
    ...
    //transition
    if (image_index >= 5) and (sprite_index == SyokaiD) and (!death_transition_done) {
